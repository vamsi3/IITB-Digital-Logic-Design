...
void readcsv(unsigned int tableout[],unsigned int p)
{
	//printf("entering readcsv\n");
	FILE* stream = fopen("/home/vighnesh/Music/20140524/makestuff/apps/flcli/network.txt", "r");
	//printf("opened file\n");
	
	unsigned int posx, posy, direction, track_ok, next_distance;
		
    while (fscanf(stream,"%u,%u,%u,%u,%u", &posx,&posy,&direction,&track_ok,&next_distance) != EOF)
    {
		//printf("reading row\n");
        if(p == 16*posx + posy)
       	{
			//printf("found position\n");
       		tableout[direction]=(1u<<7)+(1u<<6)*track_ok+(1u<<3)*direction+next_distance;
			//printf("added to table\n");
       	}
		//printf("Pos = %u, %u, %u, %u, %u\n", posx, posy,direction,track_ok,next_distance);
    }
	fclose(stream);
}

void updatecsv(unsigned int inp_posx, unsigned int inp_posy, unsigned int p) {
	unsigned int inp_next_distance = p%8; p/=8;
	unsigned int inp_direction = p%8; p/=8;
	unsigned int inp_track_ok = p%2; p/=2;
	unsigned int track_exists = p;
	char* file_path = "/home/vighnesh/Music/20140524/makestuff/apps/flcli/network.txt";
	char* temp_file_path = "/home/vighnesh/Music/20140524/makestuff/apps/flcli/temp_network.txt";
	FILE* file_stream = fopen(file_path, "r");
	FILE* temp_file_stream = fopen(temp_file_path, "w");
	unsigned int posx, posy, direction, track_ok, next_distance;
    while (fscanf(file_stream,"%u,%u,%u,%u,%u", &posx, &posy, &direction, &track_ok, &next_distance) != EOF) {
        if(inp_posx == posx && inp_posy == posy && inp_direction == direction) {
        	next_distance = inp_next_distance;
        	track_ok = inp_track_ok;
			if (track_exists == 1) fprintf(temp_file_stream,"%u,%u,%u,%u,%u\n", posx, posy, direction, track_ok, next_distance);
       	}
       	else fprintf(temp_file_stream,"%u,%u,%u,%u,%u\n", posx, posy, direction, track_ok, next_distance);
    }
    fclose(file_stream);
...
unsigned int decrypt(unsigned int p) {
	unsigned int ans,t1;
	ans = p;
	char k[] = "11001100110011001100110011000001";
	int t=0,c=8,temp,temp1=0;
	for (int i=0;i<4;i++){
		for (int j=0;(j+i)<32;j+=4){
			temp = k[j+i] - '0';
			temp1 = temp1^temp;
		}
		t = t + (temp1)*c;
...
unsigned int encrypt(unsigned int p) {
	unsigned int ans,t1;
	ans = p;
	char k[] = "11001100110011001100110011000001";
	int t=0,c=8,temp,temp1=0;
...
int main(int argc, char *argv[]) {
	...
	struct arg_lit *projectOpt  = arg_lit0("y", "custom_project_option", "                Railway Project");
	...
	void *argTable[] = {
		...
		shellOpt, benOpt, projectOpt, rstOpt, dumpOpt, helpOpt, eepromOpt, backupOpt, endOpt
	};
	...
	if ( projectOpt->count ) {
		printf("\nEntering CommFPGA Project mode:\n");
		if ( isCommCapable ) {
		   	uint8 isRunning;
			fStatus = flSelectConduit(handle, conduit, &error);
			CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
			...
			if ( isRunning ) {
				while(true)
				{
					uint8 recvData[4], sendk[4];
					uint8 positions[64][2];
					uint8 dec_ack1[4] = {110,223,183,219};
					unsigned int decpos;
					int channel=-1;
					for(int ch=0;ch<4;ch++)
					{
						printf("I = %d:\n",ch);
						printf("Received encrypted data in channel %d is ", 2*ch);
						for(int i=0;i<4;i++)
						{
							fStatus = flReadChannel(handle,2*ch,1,recvData+i,&error);
							printf("%u ", (unsigned int) recvData[i]);
						}
						printf("\n");
						decpos = decrypt(recvData[0]);
						decpos%=256u;
						printf("Position is %u, %u\n",decpos/16u,decpos%16u);
						positions[ch][0]=decpos/16u;
						positions[ch][1]=decpos%16u;



						printf("Sending encrypted position in channel %d ", 2*ch+1);
						for(int i=0;i<4;i++)
						{
							printf("%u ", (unsigned int) recvData[i]);
						}
						printf("\n");
						fStatus = flWriteChannel(handle,2*ch+1,4,recvData,&error);
						...
						while(reread<=1)
						{
							...
							printf("\n");
							printf("Decrypted ack1 is ");

							int p=0;
							for(int i=0;i<4;i++)
							{
								uint8 dec_ack = decrypt(recvData[i])%256u;
								if(dec_ack != dec_ack1[i])  
								{
									p++;
									reread++;
									if(reread==1) printf("\nAck1 is not same attempting to read again\n");
									break;
								}
								printf("%u ",(unsigned int) dec_ack);
							}
							if(reread==2) 
							{
								positions[ch][0]=-1;
								positions[ch][1]=-1;
								printf("\nDid not get Ack1, Something is wrong in I = %d\n", ch);
								break;
							}
							if(p==0)
							{
						...
						if(channel!=-1)
						{
							printf("Everything is correct in I = %d\n", channel);
							break;
						}
						if(ch == 3) ch=-1;
					}

					printf("Communicating in channels %d %d :\n", 2*channel, 2*channel + 1);
					uint8 ack2 = encrypt(240u)%256u;
					uint8 Ack2[4];
					printf("Sending encrypted ack2 ");
					for(int i=0;i<4;i++)
					{
						Ack2[i]=ack2;
						printf("%u ",(unsigned int) ack2);
					}
					printf("\n");
					fStatus = flWriteChannel(handle,2*channel+1,4,Ack2,&error);
					



					unsigned int tableout[8];
					for(int i=0;i<8;i++)
					{
						tableout[i]=(1u<<3)*((unsigned int)i);
					}
					decpos = positions[channel][0]*16u+positions[channel][1];
					readcsv(tableout,decpos);
					
					unsigned int send1=0,send2=0;
					for(int myi=0;myi<4;myi++)
					{
						send1+=(1u<<(8*myi))*tableout[myi];
						send2+=(1u<<(8*myi))*tableout[myi+4];
					}
					send1=encrypt(send1);
					send2=encrypt(send2);
					...
					fStatus = flWriteChannel(handle,2*channel+1,4,sendk,&error);
					printf("\n");
					...
					while(true)
					{
						printf("Received encrypted data is ");
						for(int i=0;i<4;i++)
						{
							fStatus = flReadChannel(handle,2*channel,1,recvData+i,&error);
							printf("%u ", (unsigned int) recvData[i]);
						}
						printf("\n");
						printf("Decrypted ack1 is ");
						...
						if(seconds1-seconds0>=256) 
						{
							printf("Waited for 256s breaking now\n");
							al++;
							break;
						}
					}
					...
					for(int i=0;i<8;i++)
					{
						fStatus = flReadChannel(handle,i+5,1,&recvData11,&error);
						printf("%d actual byte is %u but received is %u\n", i,tableout[i], (unsigned int) recvData11);
					}
					*/

					sleep(24);
					uint8 recvData1;
					fStatus = flReadChannel(handle,2*channel,1,&recvData1,&error);
					seconds0 = time(NULL);
					int ap=0;
					while(recvData1 == 128)
					{
						fStatus = flReadChannel(handle,2*channel,1,&recvData1,&error);
						time_t seconds1=time(NULL);
						if(seconds1-seconds0>=10) 
						{
							printf("Waited for 10s breaking now\n");
							ap++;
							break;
						}
					}
					if(!ap)
					{
						printf("received encrypted data from fpga is %u\n",(unsigned int) recvData1);
						recvData1 = decrypt(recvData1)%256u;
						printf("received data from fpga is %u\n",(unsigned int) recvData1);
						updatecsv(positions[channel][0],positions[channel][1],recvData1);
					}
					fStatus = flReadChannel(handle,10,1,recvData,&error);
					printf("from the view of fpga last received data_on_uart is %u ", (unsigned int) recvData[0]);
					printf("\n\n");
					sleep(10);
				}
