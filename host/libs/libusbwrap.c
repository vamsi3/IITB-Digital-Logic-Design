...
{
	printf("Entered bulk await completion now... \n");
	USBStatus retVal = USB_SUCCESS;
	struct TransferWrapper *wrapper;
	struct libusb_transfer *transfer;
	int *completed;
...
DLLEXPORT(USBStatus) usbBulkAwaitCompletionEntity(
	struct USBDevice *dev, struct CompletionReport *report, const char **error, int inp_timeout, bool *is_timeout)
{
	printf("Entered bulk await ENTITY NEW completion called... \n");
	*is_timeout = false;
	...
	int *completed;
	int iStatus;
	struct timeval timeout = {inp_timeout, 0};
	                         // This horrible thing should boil down to a call to poll() with
	                         // timeout -1ms, which will be interpreted as "no timeout" on all
	                         // platforms.
	USBStatus uStatus = queueTake(&dev->queue, (Item*)&wrapper);
	...
	completed = &wrapper->completed;
	wrapper->bufPtr = NULL;
	while ( *completed == 0 && (!(*is_timeout))) {
		iStatus = libusb_handle_events_timeout_completed(m_ctx, &timeout, completed);
		if (*completed == 0) *is_timeout = true;
		if ( iStatus < 0 ) {
			if ( iStatus == LIBUSB_ERROR_INTERRUPTED ) {
				continue;
			}
